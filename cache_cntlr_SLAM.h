...
namespace ParametricDramDirectoryMSI
{
   ...
   class CacheParameters
   {
      public:
         ...
         bool perfect;
         bool coherent;
	 ComponentLatency data_access_time; //modified..read time
         ComponentLatency tags_access_time;
         ComponentLatency writeback_time;
	 ComponentLatency STT_wr_lat;  /*modified*/
	    ...
	    , STT_wr_lat(NULL,0) /*modified*/
         ...
         CacheParameters(
            ...
            const ComponentLatency& _writeback_time, const ComponentLatency& _STT_wr_lat /*modified*/,const ComponentBandwidthPerCycle& _next_level_read_bandwidth,
         ...
         :
            ...
            writeback_time(_writeback_time), STT_wr_lat(_STT_wr_lat)/*modified*/,next_level_read_bandwidth(_next_level_read_bandwidth),
   ...
   class CacheCntlr : ::CacheCntlr
   {
      private:
         ...
         bool m_l1_mshr;
	 //////////////////////modified for pi///////////////
	 static int num_L1d_line_evic[8]; //count of each line in L1d set got evicted
	 static int num_L1d_LRU_evic_bits[8]; //max. LRU bits = 7, min = 0, count of 'each LRU bits count' for any line getting evicted from L1d 
	 static int cstateCountinEvictingSets[samples][6]; //for every set in L1d, get count of cstate in that set on eviction of M blk
	 static int num_l1d_evic; 
	 static int num_l1d_evic_samples;
	 static int num_l1d_evic_noCleanBlks; //num of evic that wont have clean blk for selecting as next LRU blk
	 static int num_actual_samples;	
	 static int num_STT_wr;
	 static int num_LLC_acc;
	/////////////////////////////////////////////////////////////////////
	 //////////////////Modified. Counters for LLC accesses////////////////////
	//static int num_LLC_rd; 
	/*static float num_LLC_acc;
	static float num_LLC_misses;
	static float num_LLC_ins;
	static float num_LLC_wr;
	static float num_LLC_pref_ins;
	static float num_LLC_pref;
	static int num_LLC_evic;
	static int wr_hit_LLC;*/
        ///////////////////////LLC wr analysis counters///////////////////////
	/*static float num_LLC_core_wr_b[4][4]; //LLC fills for bank_x by m_core_id
	static float num_LLC_cohe_wr_b[4][4]; //LLC cohe wr for bank_x by m_core_id 
	static float num_LLC_pref_wr_b[4][4]; //LLC wr for bank_x by m_core_id  
	static float num_LLC_M_ins_b[4][4]; //LLC fills for bank_x by m_core_id
	static float num_LLC_S_ins_b[4][4]; //LLC fills for bank_x by m_core_id
	static int num_L1_M_evic;
	static int num_L1_S_evic;
	static int num_L1_M_evic_bi;
	static int num_L1_S_evic_bi;*/
	//////////////////////////////////////////////////////////////////////
	std::ofstream m_myfile; /*modified for m_myfile*/
        /////////////////////////////////////////////////////////////////////
         struct {
           UInt64 loads, stores;
	 ...
	 UInt32 m_associativity; //modified
         bool m_cache_writethrough;
         ComponentLatency m_writeback_time;
	 ComponentLatency m_STT_wr_lat; /*modified*/
	 ComponentLatency m_data_access_time; /*modified..to access data_access_time and see its value*/
