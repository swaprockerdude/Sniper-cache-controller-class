...
#include "cache_base.h" //modified to use 'splitAdrs' func
#include <iostream>
#include <fstream>
#include <cstring>
...
#define num_banks 4  /*modified*/
#define sample_interval 128  /*modified*/
#define samples 98304  /*modified*/
...
int CacheCntlr::num_actual_samples = 0;
CacheCntlr::CacheCntlr(MemComponent::component_t mem_component,
   ...
   m_STT_wr_lat(cache_params.STT_wr_lat), /*modified*/
   m_data_access_time(cache_params.data_access_time), /*modified*/
   ...
   m_shmem_perf_model(shmem_perf_model)
{
   ...
   if (is_last_level_cache)
   {
	...
	m_associativity = m_master->m_cache->getAssociativity(); //modified
	
	/////////////// Getting accesses to LLC in myfile SLAM///////////////
	m_myfile.open ("/home/swapnil/sniper-6.1/LLC_access_analysis/RWEEHC/PI_1/example2.txt", std::fstream::out | std::fstream::app);
	if (! m_myfile.is_open()) {
		std::cout << "Error opening file" << std::endl;
	}
	else{
		m_myfile<<"Core: "<<m_core_id<<" Master Core: "<<m_core_id_master<<" Mem: "<<m_mem_component<<" size: "<<m_master->m_cache->getcachesize()<<" num_sets: "<<m_master->m_cache->getNumSets()<<" assoc: "<<m_associativity<<" Pointer: "<<this<<std::endl;
        } //modified
	////////////////////////////////////////////////////////////////
...
{
   if(isLastLevel()&& m_core_id == 0){
     /*for(int i=0; i<num_banks; i++){
       	 m_myfile<<"num_LLC_core_wr_b["<<i<<"]["<<m_core_id<<"] = "<<num_LLC_core_wr_b[i][m_core_id]<<std::endl;
	 m_myfile<<"num_LLC_cohe_wr_b["<<i<<"]["<<m_core_id<<"] = "<<num_LLC_cohe_wr_b[i][m_core_id]<<std::endl;
	 m_myfile<<"num_LLC_pref_wr_b["<<i<<"]["<<m_core_id<<"] = "<<num_LLC_pref_wr_b[i][m_core_id]<<std::endl;
	 m_myfile<<"num_LLC_M_ins_b["<<i<<"]["<<m_core_id<<"] = "<<num_LLC_M_ins_b[i][m_core_id]<<std::endl;
	 m_myfile<<"num_LLC_S_ins_b["<<i<<"]["<<m_core_id<<"] = "<<num_LLC_S_ins_b[i][m_core_id]<<std::endl;
     }*/
	m_myfile<<"num_STT_wr: "<<num_STT_wr<<std::endl;
	m_myfile<<"num_LLC_acc: "<<num_LLC_acc<<std::endl;
   }

   /*if(m_mem_component == 3 && m_core_id == 0){
     for(int i = 0; i<m_associativity; i++){
	m_myfile<<"Count for L1d evic at line "<<i<<": "<<num_L1d_line_evic[i]<<std::endl;
	m_myfile<<"Count for L1d LRU evic bits "<<i<<": "<<num_L1d_LRU_evic_bits[i]<<std::endl;
      } 
      m_myfile<<"L1 M evictions = "<<num_L1_M_evic<<std::endl; m_myfile<<"L1 S evictions = "<<num_L1_S_evic<<std::endl;
      m_myfile<<"L1 M evictions back inv = "<<num_L1_M_evic_bi<<std::endl; m_myfile<<"L1 S evictions back inv = "<<num_L1_S_evic_bi<<std::endl;
      m_myfile<<"LLC evictions = "<<num_LLC_evic<<std::endl;
      for(int i = 0; i<samples; i++){
	//m_myfile<<"Sample "<<i<<"\t"<<cstateCountinEvictingSets[i][0];
	for(int j = 1; j<6; j=j+2){ 
           m_myfile<<"\t"<<cstateCountinEvictingSets[i][j];
	}
	m_myfile<<std::endl;      
      }
	int i=0;
	while(!(cstateCountinEvictingSets[i][1] == 0 && cstateCountinEvictingSets[i][3] == 0 && cstateCountinEvictingSets[i][5] == 0)){
	  num_actual_samples++; i++;
	}
	m_myfile<<"Total Sampled evictions: "<<num_actual_samples<<" NoCleanBlksEvic: "<<num_l1d_evic_noCleanBlks<<std::endl;
   }*/
...
MYLOG("L1 hit");
	...
	//modified.. Here, get time when the prefetched data was used by L1..
         stats.hits_prefetch++;
         prefetch_hit = true;
...
MYLOG("access done");
   ...
   {
	////m_myfile<<" Call trainPrefetcher() fn after finishing curr req.."<<" core: "<<m_core_id<<" mem: "<<m_mem_component<<" Thread_id: "<<(Sim()->getCoreManager()->amiUserThread() ? ShmemPerfModel::_USER_THREAD : ShmemPerfModel::_SIM_THREAD)<<" adrs: "<<ca_address<<std::endl;
   ...
   {
	////m_myfile<<" Call Prefetch() fn"<<" core: "<<m_core_id<<" mem: "<<m_mem_component<<" Thread_id: "<<(Sim()->getCoreManager()->amiUserThread() ? ShmemPerfModel::_USER_THREAD : ShmemPerfModel::_SIM_THREAD)<<std::endl;
      Prefetch(t_start);
   }
...
CacheCntlr::trainPrefetcher(IntPtr address, bool cache_hit, bool prefetch_hit, SubsecondTime t_issue)
{
   ...
   if (!cache_hit || (m_prefetch_on_prefetch_hit && prefetch_hit))
   {
	...
	////m_myfile<<" In trainPrefetcher() fn..prefetch on miss.."<<" core: "<<m_core_id<<" mem: "<<m_mem_component<<" Thread_id: "<<(Sim()->getCoreManager()->amiUserThread() ? ShmemPerfModel::_USER_THREAD : ShmemPerfModel::_SIM_THREAD)<<" adrs: "<<address<<std::endl;
...
CacheCntlr::Prefetch(SubsecondTime t_now)
{
...

   {
      ...
      if (m_master->m_prefetch_next <= t_now)
      {
		...
		////m_myfile<<" In Prefetch() fn"<<" core: "<<m_core_id<<" mem: "<<m_mem_component<<" Thread_id: "<<(Sim()->getCoreManager()->amiUserThread() ? ShmemPerfModel::_USER_THREAD : ShmemPerfModel::_SIM_THREAD)<<" adrs to prefetch: "<<address<<std::endl;
   ...
   {
	////m_myfile<<" Jump to doPrefetch()--"<<" core: "<<m_core_id<<" mem: "<<m_mem_component<<" Thread_id: "<<(Sim()->getCoreManager()->amiUserThread() ? ShmemPerfModel::_USER_THREAD : ShmemPerfModel::_SIM_THREAD)<<" adrs to prefetch: "<<address_to_prefetch<<std::endl;
...
//float CacheCntlr::num_LLC_pref = 0.00; //modified..initialization of static mem var
void
 ...
 *****************************************************************************/
//float CacheCntlr::num_LLC_misses = 0.00; //modified..initialization of static mem var
//int CacheCntlr::wr_hit_LLC = 0; //modified..initialization of static mem var
//int CacheCntlr::rd_hit_LLC = 0; //modified..initialization of static mem var
HitWhere::where_t
CacheCntlr::processShmemReqFromPrevCache(CacheCntlr* requester, Core::mem_op_t mem_op_type, IntPtr address, bool modeled, bool count, Prefetch::prefetch_type_t isPrefetch, SubsecondTime t_issue, bool have_write_lock)
{
   ...
   {  
      ...
      {
	/*if(isLastLevel()){//modified
	  wr_hit_LLC = 1;
	  m_myfile<<"Wr hit in LLC!! Adrs: "<<address<<" m_core_id: "<<m_core_id<<" owner: "<<cache_block_info->getOwner()<<" cstate:"<<cache_block_info->getCState()<<std::endl;
	}*/
         ...
         #endif
	 /*if(isLastLevel() && cache_block_info!=NULL){//modified
	   m_myfile<<"Wr hit in LLC did not INV its own blk.. Adrs: "<<address<<" m_core_id: "<<m_core_id<<" owner: "<<cache_block_info->getOwner()<<" cstate:"<<cache_block_info->getCState()<<std::endl; //wr_hit_LLC = 0;
	 }*/
	 ...
	 //m_myfile<<"reading MODIFIED data.. Rd miss in L1 of core: "<<m_core_id<<" for adrs: "<<address<<std::endl;
         SubsecondTime latency = SubsecondTime::Zero();
         for(CacheCntlrList::iterator it = m_master->m_prev_cache_cntlrs.begin(); it != m_master->m_prev_cache_cntlrs.end(); it++) {
            if (*it != requester) {
		//m_myfile<<"reading MODIFIED data.. Value in (*it): "<<(*it)<<" core: "<<m_core_id<<" adrs: "<<address<<" mem: "<<m_mem_component<<std::endl;
      ...
      else if (cache_block_info->getCState() == CacheState::EXCLUSIVE) // reading EXCLUSIVE data
      {
         ...
         SubsecondTime latency = SubsecondTime::Zero();
	 //m_myfile<<"reading EXCLUSIVE data.. Rd miss in L1 of core: "<<m_core_id<<" for adrs: "<<address<<std::endl;
         for(CacheCntlrList::iterator it = m_master->m_prev_cache_cntlrs.begin(); it != m_master->m_prev_cache_cntlrs.end(); it++) {
            if (*it != requester) {
		//m_myfile<<"reading EXCLUSIVE data.. Value in (*it): "<<(*it)<<" core: "<<m_core_id<<" adrs: "<<address<<" mem: "<<m_mem_component<<std::endl;
               std::pair<SubsecondTime, bool> res = (*it)->updateCacheBlock(address, CacheState::SHARED, Transition::COHERENCY, NULL, ShmemPerfModel::_USER_THREAD);
               latency = getMax<SubsecondTime>(latency, res.first);
               sibling_hit |= res.second;
            }
   ...
   if (modeled && m_master->m_prefetcher)
   {
      ////m_myfile<<" Call trainPrefetcher() fn"<<" core: "<<m_core_id<<" mem: "<<m_mem_component<<" Thread_id: "<<(Sim()->getCoreManager()->amiUserThread() ? ShmemPerfModel::_USER_THREAD : ShmemPerfModel::_SIM_THREAD)<<" adrs: "<<address<<std::endl;
...
CacheCntlr::processExReqToDirectory(IntPtr address)
{
   ...
   getMemoryManager()->sendMsg(PrL1PrL2DramDirectoryMSI::ShmemMsg::EX_REQ,
         ...
         m_core_id_master /* requester MODIFIED FOR LAP to m_core_id from m_core_id_master */,
...
//float CacheCntlr::num_LLC_wr = 0.00; //modified..initialization of static mem var
void
...
int CacheCntlr::num_LLC_acc = 0; //modified..initialization of static mem var
void
CacheCntlr::retrieveCacheBlock(IntPtr address, Byte* data_buf, ShmemPerfModel::Thread_t thread_num, bool update_replacement)
{
   ...
   if(isLastLevel())
      num_LLC_acc++;//modified for calc dyn energy of LLC
}
 ...
 *****************************************************************************/
//int CacheCntlr::num_LLC_evic = 0; //modified..initialization of static mem var
//int CacheCntlr::num_L1_M_evic = 0; //modified..initialization of static mem var
//int CacheCntlr::num_L1_S_evic = 0; //modified..initialization of static mem var
//float CacheCntlr::num_LLC_ins = 0.00; //modified..initialization of static mem var
//float CacheCntlr::num_LLC_core_wr_b[num_banks][4] = {{0}};
//float CacheCntlr::num_LLC_cohe_wr_b[num_banks][4] = {{0}};
int CacheCntlr::num_STT_wr = 0;
int CacheCntlr::num_L1d_line_evic[8] = {0}; //count of each line in L1d set got evicted
int CacheCntlr::num_L1d_LRU_evic_bits[8] = {0}; //max. LRU bits = 7, min = 0, count of 'each LRU bits count' for any line getting evicted from L1d 
int CacheCntlr::cstateCountinEvictingSets[samples][6] = {{0}}; //SLAM 
int CacheCntlr::num_l1d_evic = 0; //SLAM
int CacheCntlr::num_l1d_evic_samples = 0; //SLAM
int CacheCntlr::num_l1d_evic_noCleanBlks = 0;
SharedCacheBlockInfo*
...
MYLOG("insertCacheBlock l%d @ %lx as %c (now %c)", m_mem_component, address, CStateString(cstate), CStateString(getCacheState(address)));
   ...
   UInt32 l_LLC = 0;//SLAM
   IntPtr L1_evict_adrs;//SLAM

   LOG_ASSERT_ERROR(getCacheState(address) == CacheState::INVALID, "we already have this line, can't add it again");
   //////////////SLAM tracking the MODIFIED blocks for eviction///////////////////
   if(m_mem_component == 3 && num_l1d_evic%sample_interval == 0 && num_l1d_evic_samples<samples){//only on sampled L1d evic apply PI
     //fetch adrs of rep index to see if same adrs in LLC is in STT reg
     L1_evict_adrs = m_master->m_cache->fetchAdrsOfRepIndex(address, this);
     if(L1_evict_adrs != NULL && m_next_cache_cntlr->m_master->m_cache->getLineIndex(L1_evict_adrs)>3){//get line index of L1_evict_adrs in LLC adn make sure it is in STT reg
       l_LLC = m_next_cache_cntlr->m_master->m_cache->getLineIndex(L1_evict_adrs);
       m_master->m_cache->insertSingleLineLLC(l_LLC, address, data_buf,
	...
	m_master->m_cache->insertSingleLine(address, data_buf,
         &eviction, &evict_address, &evict_block_info, evict_buf,
         getShmemPerfModel()->getElapsedTime(thread_num), this); //orig is only this line
     }
   }   
   else{
     m_master->m_cache->insertSingleLine(address, data_buf,
  ...
  SharedCacheBlockInfo* cache_block_info = setCacheState(address, cstate);
   ...
   if(isLastLevel()){
      num_LLC_acc++;//modified for calc dyn energy of LLC
     if(m_master->m_cache->getLineIndex(address) > 3)){//STT reg
	  SubsecondTime lat = m_STT_wr_lat.getLatency(); getShmemPerfModel()->incrElapsedTime(lat, thread_num); //Delay for WR to STT
	  num_STT_wr++;
     }
   }
...
MYLOG("insertCacheBlock l%d local done", m_mem_component);
   ...
   {             
	...
	///////////////////SLAM customizing MESI protocol to minimize write-back operations///////////////
	if(m_mem_component == 3 && evict_block_info.getCState() == CacheState::MODIFIED){ //if L1d
	  if(num_l1d_evic%sample_interval == 0 && num_l1d_evic_samples<samples){
	    //m_myfile<<"Eviction at line: "<<m_master->m_cache->getLineIndex(address)<<" LRU bits: "<<evict_block_info.getlrubits()<<std::endl;
	    /*UInt32 restLRUbits[m_associativity]; CacheState::cstate_t restCstate[m_associativity];
	    m_master->m_cache->getrestLRUbits(address, restLRUbits, restCstate); //get rest lru bits in array of size assoc
	    cstateCountinEvictingSets[num_l1d_evic_samples][0] = m_master->m_cache->getsetindex(address);
	    for(int i = 0; i < m_associativity; i++){
	       //m_myfile<<"LRU bits count at line "<<i<<" are: "<<restLRUbits[i]<<" cstate: "<<restCstate[i]<<std::endl;
	      if(i != m_master->m_cache->getLineIndex(address))//dont consider inserted line's cstate
	        cstateCountinEvictingSets[num_l1d_evic_samples][restCstate[i]]++;
	    }
	    if(cstateCountinEvictingSets[num_l1d_evic_samples][5] == 7)
		num_l1d_evic_noCleanBlks++;*/
	    num_l1d_evic_samples++;//we need only 128 evic samples
	  }	
	   /*num_L1d_line_evic[m_master->m_cache->getLineIndex(address)]++; //SLAM
	   num_L1d_LRU_evic_bits[evict_block_info.getlrubits()]++; //SLAM*/
	 num_l1d_evic++;
	}	
	/////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      else if (m_next_cache_cntlr)
      {
		...
		if (evict_block_info.getCState() == CacheState::MODIFIED)
		  m_next_cache_cntlr->writeCacheBlock(evict_address, 0, evict_buf, getCacheBlockSize(), thread_num); //ORIGINAL IS ONLY THIS LINE with above 'if'
		////////////////////modified for LLC wr analysis/////////////////////
		  /*if(m_mem_component == 3){//M L1 evic
	              //num_L1_M_evic++;
		      //UInt32 bank_num = ((m_next_cache_cntlr->m_master->m_cache->getsetindex(evict_address)) / (m_next_cache_cntlr->m_master->m_cache->getNumSets()/num_banks)); //get bank num for foll.
		      //num_LLC_cohe_wr_b[bank_num][m_core_id]++;
		      m_myfile<<"M L1 evic from line: "<<m_master->m_cache->getLineIndex(address)<<" LRU bits: "<<evict_block_info.getlrubits()<<std::endl;
	           }
		}
	       else{//clean L1 evic
		  if(m_mem_component == 3){
			m_myfile<<"S L1 evic from line: "<<m_master->m_cache->getLineIndex(address)<<" LRU bits: "<<evict_block_info.getlrubits()<<std::endl; 
                        //num_L1_S_evic++; 
		  }
	       }*/
	       //////////////////////////////////////////////////////////////////////////// 	     
         }//End of Send dirty block to next level cache else
...
//float CacheCntlr::num_LLC_cohe_wr_b[4][4] = {{0}};
//int CacheCntlr::num_L1_M_evic_bi = 0;
//int CacheCntlr::num_L1_S_evic_bi = 0;
std::pair<SubsecondTime, bool>
CacheCntlr::
updateCacheBlock(IntPtr address, CacheState::cstate_t new_cstate, Transition::reason_t reason, Byte* out_buf, ShmemPerfModel::Thread_t thread_num)
{
   ...
   if (! m_master->m_prev_cache_cntlrs.empty())
   {
      for(CacheCntlrList::iterator it = m_master->m_prev_cache_cntlrs.begin(); it != m_master->m_prev_cache_cntlrs.end(); it++) {
         std::pair<SubsecondTime, bool> res = (*it)->updateCacheBlock(
            address, new_cstate, reason == Transition::EVICT ? Transition::BACK_INVAL : reason, out_buf/*MODIFIED FOR LAP..orig NULL*/, thread_num);
   ...
   else
   {
      ...
      if (cache_block_info->getCState() == CacheState::MODIFIED) {
	    ...
	    m_next_cache_cntlr->writeCacheBlock(address, 0, data_buf, getCacheBlockSize(), thread_num); //ORIGINAL IS ONLY THIS LINE
	    is_writeback = true; sibling_hit = true; //ORIGINAL IS ONLY THIS LINE
	    ////////////////////modified for LLC wr analysis/////////////////////
		    /*UInt32 bank_num = ((m_next_cache_cntlr->m_master->m_cache->getsetindex(address)) / (m_next_cache_cntlr->m_master->m_cache->getNumSets()/num_banks)); //get bank num for foll.
		    num_LLC_cohe_wr_b[bank_num][m_core_id]++;*/
 	    /////////////////////////////////////////////////////////////////////
...
//float CacheCntlr::num_LLC_pref_ins = 0.00; //modified..initialization of static mem var
//float CacheCntlr::num_LLC_pref_wr_b[num_banks][4] = {{0}};
//float CacheCntlr::num_LLC_M_ins_b[num_banks][4] = {{0}};
//float CacheCntlr::num_LLC_S_ins_b[num_banks][4] = {{0}};
void
...
MYLOG("WB REQ<%u @ %lx", sender, address);
         ...
         || (shmem_msg_type == PrL1PrL2DramDirectoryMSI::ShmemMsg::UPGRADE_REP) )
   {
	...
	//UInt32 bank_num = ((m_master->m_cache->getsetindex(address)) / (m_master->m_cache->getNumSets()/num_banks)); //get bank num for foll.
        if (request->isPrefetch){//modified.. pref_wr
            getCacheBlockInfo(address)->setOption(CacheBlockInfo::PREFETCH); 
	}
	/////////////////modified for LLC wr analysis///////////////
        /*    num_LLC_pref_wr_b[bank_num][m_core_id]++;
	}//end of pref_wr
	else{//core_wr
	  num_LLC_core_wr_b[bank_num][m_core_id]++;
	}*/          
